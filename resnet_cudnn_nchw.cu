...
Forward_Buffer * init_forward_buffer(Dims * dims, ConvBlock ** conv_blocks, int batch_size){
	...
	int * forward_conv_algos = (int *) malloc(256 * sizeof(int));
	for (int i = 0; i < 52; i++){
...
Backprop_Buffer * init_backprop_buffer(Dims * dims, ConvBlock ** conv_blocks, int batch_size){
	...
	int * data_algos = (int *) malloc(52 * sizeof(int));
	int * filt_algos = (int *) malloc(52 * sizeof(int));
	for (int i = 0; i < 52; i++){
		data_algos[i] = -1;
		filt_algos[i] = -1;
	}
...
void prepareAndDoMaxPool(Train_ResNet * trainer, const float * input, int in_spatial_dim, int filters, int kern_dim, int stride, int batch_size, float * out){
	...
	cudnnPoolingForward(trainer -> cudnnHandle, pool_descriptor, &alpha, input_descriptor, input, &beta, out_descriptor, out);
...
void prepareAndDoMaxPoolDeriv(Train_ResNet * trainer, const float * input, float * output, float * out_layer_deriv, int in_spatial_dim, int filters, int kern_dim, int stride, int batch_size, float * input_deriv){
	...
	cudnnPoolingBackward(trainer -> cudnnHandle, &alpha, out_descriptor, output, out_descriptor, out_layer_deriv, input_descriptor, input, &beta, input_descriptor, input_deriv);
